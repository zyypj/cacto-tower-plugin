...
public class ItemBuilder {
    ...
    public ItemBuilder(Material material, int amount, short dataID) {
        if(material == null) material = Material.AIR;
        if(((amount > material.getMaxStackSize()) || (amount <= 0)) && (!unsafeStackSize)) amount = 1;
        this.amount = amount;
        this.item = new ItemStack(material, amount, dataID);
        this.material = material;
        this.dataID = dataID;
    }
    ...
    public ItemStack build() {
        ...
        if(displayname != null) {
            meta.setDisplayName(Text.colorTranslate(displayname));
        }
        if(lore.size() > 0) {
            meta.setLore(Text.listReplace(lore, "&", "§"));
        }
        if(flags.size() > 0) {
